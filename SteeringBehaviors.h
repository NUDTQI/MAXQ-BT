...
//
//  Desc:   class to encapsulate steering behaviors for a Agent
//
//  Author: modified by Zhang Qi 2016
//
//------------------------------------------------------------------------
#include <vector>
#include <string>
...
private:
  ...
  Agent*     m_pTargetAgent1;
  Agent*     m_pTargetAgent2;
  ...
  //the distance (squared) a Agent has to be from a path waypoint before
  ...
  //Arrive makes use of these to determine how quickly a Agent
  ...
  //Vector2D OffsetPursuit(const Agent* agent, const Vector2D offset);

  //this behavior attempts to evade a pursuer
  Vector2D Evade(const Agent* agent);
  ...
  //this results in a steering force that attempts to steer the Agent
  //to the center of the vector connecting two moving agents.
  //Vector2D Interpose(const Agent* AgentA, const Agent* AgentB);

  //given another agent position to hide from and a list of BaseGameEntitys this
  //method attempts to put an obstacle between itself and its opponent
  //Vector2D Hide(const Agent* hunter, const std::vector<BaseGameEntity*>& obstacles);


  // -- Group Behaviors -- //

  Vector2D Cohesion(const std::vector<Agent*> &agents);
  
  Vector2D Separation(const std::vector<Agent*> &agents);

  Vector2D Alignment(const std::vector<Agent*> &agents);

  //the following three are the same as above but they use cell-space
  //partitioning to find the neighbors
  //Vector2D CohesionPlus(const std::vector<Agent*> &agents);
  //Vector2D SeparationPlus(const std::vector<Agent*> &agents);
  //Vector2D AlignmentPlus(const std::vector<Agent*> &agents);
...
public:
  ...
  void      SetTargetAgent1(Agent* Agent){m_pTargetAgent1 = Agent;}
  void      SetTargetAgent2(Agent* Agent){m_pTargetAgent2 = Agent;}
  Agent*    GetTargetAgent1(){return m_pTargetAgent1;}
  Agent*    GetTargetAgent2(){return m_pTargetAgent2;}
  ...
  void PursuitOn(Agent* v){m_iFlags |= pursuit; m_pTargetAgent1 = v;}
  void EvadeOn(Agent* v){m_iFlags |= evade; m_pTargetAgent1 = v;}
  ...
  void InterposeOn(Agent* v1, Agent* v2){m_iFlags |= interpose; m_pTargetAgent1 = v1; m_pTargetAgent2 = v2;}
  void HideOn(Agent* v){m_iFlags |= hide; m_pTargetAgent1 = v;}
  void OffsetPursuitOn(Agent* v1, const Vector2D offset){m_iFlags |= offset_pursuit; m_vOffset = offset; m_pTargetAgent1 = v1;}  
  ...
  void HavenAvoidanceOn(){m_iFlags |= haven_avoidance;}
  void HavenAvoidanceOff(){if(On(haven_avoidance)) m_iFlags ^= haven_avoidance;}
  bool isHavenAvoidanceOn(){return On(haven_avoidance);}
